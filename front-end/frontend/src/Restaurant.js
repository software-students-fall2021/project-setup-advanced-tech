...
const Restaurant = (props) => {
  ...
  useEffect(() => {
    ...
    console.log('fetching specific restaurant...')
    axios(`https://my.api.mockaroo.com/restaurant/${props.id}.json?key=a77dd4e0`)
      .then((response) => {
  ...
  return (
    ...
    <div>
      <div className="section">
        <p>{data.name}<Button className="button">Collect</Button></p>
      </div>
      <div className="section">
        <div>Collections:{data.collection}</div>
        <div className="city">{data.city},{data.state}</div>
      </div>
